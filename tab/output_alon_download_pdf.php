...
$pdf="admin_download_pdf/".$rollno."_".$bookno."_from_".$from_dd."-".$from_mm."-".$from_yy."_to_".$to_dd."-".$to_mm."-".$to_yy."_".time().".pdf"; 
										
$_SESSION['del_file'] = $fname ; // to delete file
$_SESSION['del_pdf'] = $pdf ;

$from_dd = $_GET['from_dd'] ;
$from_mm = $_GET['from_mm'] ;
$from_yy = $_GET['from_yy'] ;
	...
	$sql = "SELECT ROLL_NO,BOOK_NO,UNIX_TIMESTAMP(TAKEN_DATE)  AS UNIXDATE
			FROM BOOKS_TAKEN 
				WHERE   BOOK_NO = '{$bookno}' 
					ORDER BY ROLL_NO" ;		//-------- when only bookno is given
}
	...
	$sql = "SELECT ROLL_NO,BOOK_NO,UNIX_TIMESTAMP(TAKEN_DATE)  AS UNIXDATE 
			FROM BOOKS_TAKEN 
				WHERE BOOK_NO !='' AND ROLL_NO = '{$rollno}' 
				   ORDER BY ROLL_NO" ;		//-------- when only rollno is given
}
      ...
      if($status == 0)
	{
	  $test .= "<div class='container-fluid '>
		...
		<table class='table table-bordered'>
 		  <tr>
			<th style='color:black; text-align:center;'>S.No</th>
			<th style='color:black; text-align:center;'>Roll No</th>
			<th style='color:black; text-align:center;'>Book No</th>
			<th style='color:black; text-align:center;'>Date</th>
			<th style='color:black; text-align:center;'>Time</th>
...
<!-- downloading the details file(.csv)-->


<div class="container"  >
	<div id="pos_fixed">
	<p style="font-family:Ubuntu; font-size:18px;">Total number of student records found : <?php echo $sno_count ; ?> </p>
	<p style="font-family:Ubuntu; font-size:18px;">Total number of records found : <?php echo $total_records ; ?> </p>
	<a href="<?php echo $fname; ?>" class="btn btn-info ">Download excel file <span class="glyphicon glyphicon-download"></span></a></br></br>
	<a href="<?php echo $pdf; ?>" class="btn btn-info " target="_blank">Download pdf file <span class="glyphicon glyphicon-download"></span></a></br>
